...
import 'package:ebidan/common/extensions/ext.dart';
...
class KtpExtractor {
  ...
  static Future<File?> cropImageForKtp(File imageFile) async {
    final modelPath = await Utils.getAssetPath(
    ...
    for (final object in result) {
      ...
      if (object.labels.firstOrNull?.text == "Driver's license") {
        ...
        imageCropped = await Utils.cropImage(
  ...
  //         mrzCropped = await Utils.cropPassportMrz(
  //           File(inputImage.filePath!),
  //           object,
  //         );
  //         break;
