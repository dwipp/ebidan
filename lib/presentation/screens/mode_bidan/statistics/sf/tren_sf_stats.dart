...
class TrenSfStatsScreen extends StatelessWidget {
  final List<String> monthKeys;
  const TrenSfStatsScreen({super.key, required this.monthKeys});
  ...
  List<TrenIndicator> _generateSfIndicators() {
    ...
    return sfKeys.map((key) {
      final label = "SF${int.parse(key) ~/ 30}";
      final fieldName = key;
